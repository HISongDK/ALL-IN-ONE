...
3. 按照命令行提示配置 eslint 初始内容
4. 安装 eslint-plugin-react 等相关依赖

   ```js
     npm i babel-eslint eslint-plugin-react eslint-plugin-import -D
   ```

5. `.eslintrc.js`文件详细配置
...
7. 检查 Git 提交的代码：
   ...
   1. 安装`pre-commit`,一边检查提交操作：

      ```js
        npm i pre-commit -D
      ```

   2. 在`package.json`中增加以下配置：
      ...
      这里的`lint`对应第六步中脚本命令名。

完成之后，在每次代码提交之，`pre-commit`都会拦截 Git 的 commit 操作，然后运行 lint 命令进行代码检查，若检测到违反校验规则的情况，会返回错误，导致 git commit 失败。

![pre-commit-eslint校验效果](pre-commit-eslint.png)

**[<P align=center>原文地址</P>](https://zhuanlan.zhihu.com/p/84329603)**
